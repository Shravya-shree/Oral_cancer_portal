...
# Image Pre-screening Validation
def validate_oral_cavity_image(image_path):
    img = cv2.imread(image_path)
    if img is None:
        return False
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    # Mask for tissue/pinkish/reddish color ranges in HSV
    ...
    ratio = np.sum(tissue_mask > 0) / (img.shape[0] * img.shape[1])
    return ratio > 0.15  # At least 15% tissue color characteristics required
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"\n[!] MYSQL LOGIN ERROR: {e}\n")
            flash('Database connection failed. Check your terminal for the exact error.', 'danger')
...
def scan():
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ...
            # Generate QR Code for result summary
